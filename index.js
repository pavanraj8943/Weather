    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      try {
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=afe66245bd764aaaac3a5196ef5c2d17`
        );
        if (res.status === 200) {
        ...
        console.log("Location weather fetch failed", err);
      }
    });
  }
});
